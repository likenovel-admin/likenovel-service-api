...
update tb_product_episode
   set open_yn = 'Y'
     , open_changed_date = @release_now
     , updated_id = 0
     , updated_date = @release_now
 where publish_reserve_date IS NOT NULL
   and publish_reserve_date <= @release_now
...
update tb_product p
 inner join (
   ...
   set p.open_yn = 'Y'
     , p.last_episode_date = @release_now
     , p.updated_id = 0
     , p.updated_date = @release_now
 where p.open_yn = 'N'
   and p.blind_yn = 'N'
;
...
update tb_product p
 inner join (
    select distinct e.product_id
      from tb_product_episode e
     where e.open_changed_date = @release_now
       and e.open_yn = 'Y'
       and e.use_yn = 'Y'
 ) pe on pe.product_id = p.product_id
   set p.last_episode_date = @release_now
     , p.updated_date = @release_now
 where p.open_yn = 'Y'
;
