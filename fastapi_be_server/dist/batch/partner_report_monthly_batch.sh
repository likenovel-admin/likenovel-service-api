...
BATCH_NAME="partner_report_monthly_batch"
...
for attempt in $(seq 1 $MAX_RETRIES); do
  mysql -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PW" "$DB_NAME" --default-character-set=utf8mb4 $MYSQL_SSL_OPT < "${SCRIPT_DIR}/partner_report_monthly_batch.sql"
  rc=$?
  if [ $rc -eq 0 ]; then
    exit 0
  fi
