...
for _try in 1 2 3; do
  if [ -r "$ENV_FILE" ] && [ -s "$ENV_FILE" ]; then
    while IFS="=" read -r key value || [ -n "$key" ]; do
      case "$key" in
      DB_HOST|DB_IP|DB_PORT|DB_USER|DB_PW|DB_USER_ID|DB_USER_PW|DB_NAME|ANTHROPIC_API_KEY|ANTHROPIC_MODEL|OPENROUTER_API_KEY|OPENROUTER_BASE_URL|OPENROUTER_BACKGROUND_RESERVE_USD|OPENROUTER_BACKGROUND_IN_FLIGHT_BUFFER_USD|STORY_AGENT_SUMMARY_MODEL|STORY_AGENT_SUMMARY_TEMPERATURE|STORY_AGENT_RP_REASONING_MODEL|STORY_AGENT_RP_REASONING_EFFORT|STORY_AGENT_RP_REASONING_THINKING_DISPLAY|STORY_AGENT_CHARACTER_SIGNALS_OPENROUTER_MODEL|STORYCTX_MAX_PARALLEL|PUBLIC_CHARACTER_CATALOG_SNAPSHOT_AUTO_REFRESH_ENABLE|AI_DNA_PROVIDER|AI_DNA_OPENROUTER_MODEL|AI_DNA_OPENROUTER_PROVIDER_ONLY|AI_DNA_RESPONSE_FORMAT|AI_DNA_TIMEOUT_SECONDS|AI_METADATA_MAX_TOKENS|AI_METADATA_PIPELINE_VERSION|AI_METADATA_FAILED_RETRY_COOLDOWN_DAYS|AI_METADATA_INCOMPLETE_RETRY_COOLDOWN_DAYS)
          export "$key=$value"
          ;;
      esac
    done < "$ENV_FILE"
