...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "[WARN] ai_engagement_metrics_daily_batch already running ($LOCK_DIR exists), skipping." 1>&2
  exit 0
fi
...
for attempt in $(seq 1 $MAX_RETRIES); do
  ...
  echo "[WARN] ai_engagement_metrics_daily_batch attempt $attempt/$MAX_RETRIES failed (exit=$rc)" 1>&2
  if [ $attempt -lt $MAX_RETRIES ]; then
    sleep $RETRY_DELAY
  fi
done

echo "[ERROR] ai_engagement_metrics_daily_batch failed after $MAX_RETRIES attempts" 1>&2
exit 1
