...
BATCH_NAME="ai_product_episode_dropoff_daily_batch"
...
for attempt in $(seq 1 $MAX_RETRIES); do
  mysql -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PW" "$DB_NAME" --default-character-set=utf8mb4 $MYSQL_SSL_OPT < "${SCRIPT_DIR}/ai_product_episode_dropoff_daily_batch.sql"
  rc=$?
  if [ $rc -eq 0 ]; then
    exit 0
  fi
  echo "[WARN] ai_product_episode_dropoff_daily_batch attempt $attempt/$MAX_RETRIES failed (exit=$rc)" 1>&2
  if [ $attempt -lt $MAX_RETRIES ]; then
    sleep $RETRY_DELAY
  fi
done

echo "[ERROR] ai_product_episode_dropoff_daily_batch failed after $MAX_RETRIES attempts" 1>&2
exit 1
