...
- AI DNA 정기 배치와 CMS 재분석은 OpenRouter-only로 실행하며 Anthropic fallback을 사용하지 않는다.
- DNA 정기 배치는 `AI_DNA_OPENROUTER_PROVIDER_ONLY`가 비어 있으면 자동 라우팅하고, 기본 `AI_DNA_RESPONSE_FORMAT` 값으로 `json_schema`를 사용한다.
