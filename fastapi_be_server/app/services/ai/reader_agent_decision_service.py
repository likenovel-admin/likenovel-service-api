...
READER_DECISION_PROMPT_VERSION = "ai-reader-decision-v4"
...
- 작품 DNA 태그, 제목, product.early_episode_summaries, episode.current_episode_summary_text, 이전 행동 상태, 현재 공개 지표를 참고한다.
- product.early_episode_summaries는 storyctx가 만든 1~10화 회차요약 목록이다. 값이 있으면 작품의 초반 훅과 전개 흐름을 판단하는 주요 근거로 사용한다.
- episode.current_episode_summary_text는 현재 episode.episode_id에 대응하는 storyctx 회차요약이다. 값이 있으면 현재 N화 사건과 전개의 직접 근거로 storyctx 초반 회차요약과 함께 사용한다.
...
- 현재 회차 내용 근거가 없으면 독자 취향, 작품 DNA와 storyctx 초반 회차요약, 이전 행동 상태, engagement_context만 조건부확률 evidence로 사용한다.
