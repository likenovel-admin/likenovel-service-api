...
# Generic community-style seeds transformed from reference nickname patterns.
AI_READER_PROFILE_NICKNAME_KOREAN_BASE_POOL = (
    "커피두잔",
    "모니터불빛",
    "비오는점심",
    "막차기다림",
...
AI_READER_PROFILE_NICKNAME_LATIN_BASE_POOL = (
    "blueNote",
    "mossTea",
    "paperMoon",
    "quietSun",
...
AI_READER_PROFILE_NICKNAME_VARIANT_SUFFIXES = (
    "",
    "7",
    "11",
    "21",
...
AI_READER_PROFILE_NICKNAME_BLOCKED_TERMS = (
    "라이크독자",
    "독자",
    "봇",
    "운영",
...
AI_READER_PROFILE_NICKNAME_REFERENCE_EXACT_BLOCKLIST = (
    "한스콜에스",
    "프록시마B",
    "세비허",
    "OLDBOY",
...
def _build_ai_reader_profile_nickname_pool() -> tuple[str, ...]:
    pool: list[str] = []
    seen: set[str] = set()
    forbidden_terms = (
        *admin_schema.FORBIDDEN_AI_READER_NICKNAME_TERMS,
        *AI_READER_PROFILE_NICKNAME_BLOCKED_TERMS,
    )
    for nickname in [
        *AI_READER_PROFILE_NICKNAME_KOREAN_BASE_POOL,
        *(
            f"{base_nickname}{suffix}"
            for suffix in AI_READER_PROFILE_NICKNAME_VARIANT_SUFFIXES
            for base_nickname in AI_READER_PROFILE_NICKNAME_LATIN_BASE_POOL
        ),
    ]:
        ...
        if (
            nickname in seen
            or nickname in AI_READER_PROFILE_NICKNAME_REFERENCE_EXACT_BLOCKLIST
            or len(nickname) > 30
            or not admin_schema.AI_READER_NICKNAME_PATTERN.match(nickname)
            or any(term.lower() in lowered_nickname for term in forbidden_terms)
        ):
            continue
        pool.append(nickname)
        seen.add(nickname)
    return tuple(pool)


AI_READER_PROFILE_NICKNAME_POOL = _build_ai_reader_profile_nickname_pool()
...
def _activity_pattern_with_auto_pause_after(
    ...
    activity_schedule_mode: str = "uniform",
) -> dict[str, Any]:
    pattern = _parse_json_field(pattern_value, {})
    if not isinstance(pattern, dict):
        pattern = {}
    pattern["activity_schedule_mode"] = activity_schedule_mode
...
def build_ai_reader_bootstrap_dry_run_token(
    *,
    ...
    payload = {
        ...
        "activity_schedule_mode": activity_schedule_mode,
        "time_blocks": time_blocks,
        "product_type_weights": _product_type_weights_payload(product_type_weights),
        "free_product_type_weights": _free_product_type_weights_payload(
            free_product_type_weights
...
def _expected_bootstrap_dry_run_token(
    ...
    *,
    ...
    return build_ai_reader_bootstrap_dry_run_token(
        ...
        activity_schedule_mode=req_body.activity_schedule_mode,
...
def _resume_activity_pattern_for_agent(
    ...
    *,
    ...
    if req_body.activity_schedule_mode == "age_group":
        generated_pattern = reader_agent_persona_service.build_activity_pattern(
            age_group=str(agent.get("age_group") or "30s"),
            gender=str(agent.get("gender") or "X"),
            seed=int(agent["ai_reader_agent_id"]),
    ...
    if req_body.activity_schedule_mode == "uniform":
...
async def bootstrap_ai_reader_agents(
    *,
    ...
    requested_time_blocks = (
        None
        if req_body.activity_schedule_mode == "age_group"
        else _time_blocks_payload(req_body.time_blocks)
    )
    ...
    preview = [
        {
            ...
            "activity_pattern": _activity_pattern_with_auto_pause_after(
                ...
                req_body.free_product_type_weights,
                req_body.product_status_weights,
                req_body.activity_schedule_mode,
            ),
        }
        for user, seed in zip(initial_users, initial_seeds, strict=True)
    ]
