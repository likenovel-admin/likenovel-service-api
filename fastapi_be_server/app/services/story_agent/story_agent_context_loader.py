...
STORY_AGENT_SCOPE_SUMMARY_LIMIT = 6
STORY_AGENT_SCOPE_CHARACTER_LIMIT = 12
STORY_AGENT_SCOPE_RELATION_LIMIT = 16
STORY_AGENT_SCOPE_HOOK_LIMIT = 8
...
def _finalize_scoped_characters(character_map: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return rows[:STORY_AGENT_SCOPE_CHARACTER_LIMIT]
...
def _finalize_scoped_relations(relation_map: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return rows[:STORY_AGENT_SCOPE_RELATION_LIMIT]
...
def _aggregate_scoped_signal_rows(signal_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    for row in signal_rows:
        ...
        for hook in payload.get("cliffhanger_hooks") or []:
            ...
            if len(hooks) >= STORY_AGENT_SCOPE_HOOK_LIMIT:
                break
    ...
    return (
        ...
        hooks[:STORY_AGENT_SCOPE_HOOK_LIMIT],
    )


async def load_story_agent_scope_context(
    *,
    product_id: int,
    read_episode_to: int,
    ...
    summary_result = await db.execute(
        ...
        ),
        {
            ...
            "limit": STORY_AGENT_SCOPE_SUMMARY_LIMIT,
        },
    )
...
def build_story_agent_scope_context_message(scope_context: dict[str, Any]) -> str:
    ...
    if plot_rows:
        ...
        for row in plot_rows[:3]:
            ...
            blocks.append(f"- {label}: {summary_text.splitlines()[0][:180]}")

    if characters:
        blocks.append("[반복 등장 인물 신호]")
    ...
    if relations:
        ...
        for item in relations[:6]:
            ...
            if source_name and target_name:
                blocks.append(f"- {source_name} -> {target_name}: {tags}")
