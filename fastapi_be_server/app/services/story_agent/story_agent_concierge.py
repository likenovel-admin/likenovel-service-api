...
STORY_AGENT_CONCIERGE_REASON_LIMIT = 3
STORY_AGENT_CONCIERGE_TOPIC_MATCH_KEYWORDS = (
    "왜 나한테 맞",
    "왜 나랑 맞",
    "내 취향",
    "취향",
...
STORY_AGENT_CONCIERGE_TOPIC_MOOD_KEYWORDS = (
    "분위기",
    "무드",
    "느낌",
    "결이",
...
STORY_AGENT_CONCIERGE_TOPIC_ENTRY_KEYWORDS = (
    "입문",
    "왜 봐야",
    "영업",
    "시작",
...
STORY_AGENT_CONCIERGE_TOPIC_SPOILER_KEYWORDS = (
    "결말",
    "반전",
    "흑막",
    "정체",
...
def _normalize_story_agent_concierge_text(raw_value: Any) -> str:
    return str(raw_value or "").strip()


def _contains_story_agent_concierge_keyword(
    content: str,
    keywords: tuple[str, ...],
) -> bool:
...
async def _get_story_agent_recent_read_titles(
    *,
    user_id: int | None,
    product_id: int,
    db: AsyncSession,
...
def _append_story_agent_reason_card(
    cards: list[StoryAgentReasonCard],
    *,
    title: str,
    description: str,
) -> None:
...
def _normalize_story_agent_tag_list(raw_value: Any, *, limit: int = 3) -> list[str]:
    tags: list[str] = []
...
def _build_story_agent_concierge_reason_cards(
    *,
    product_row: dict[str, Any],
    product_metadata: dict[str, Any] | None,
    taste_match: dict[str, Any] | None,
    recent_read_titles: list[str],
) -> list[StoryAgentReasonCard]:
    cards: list[StoryAgentReasonCard] = []

    mood = str((product_metadata or {}).get("mood") or "").strip()
    protagonist_type = str((product_metadata or {}).get("protagonist_type") or "").strip()
    taste_tags = _normalize_story_agent_tag_list((product_metadata or {}).get("taste_tags"))
    worldview_tags = _normalize_story_agent_tag_list((product_metadata or {}).get("worldview_tags"))
    ...
    if top_axis:
        ...
        if axis == "protagonist" and protagonist_type:
            _append_story_agent_reason_card(
                cards,
                title="취향 적중",
    ...
    return cards[:STORY_AGENT_CONCIERGE_REASON_LIMIT]


def _resolve_story_agent_concierge_topic(user_prompt: str | None) -> str:
    normalized_prompt = _normalize_story_agent_concierge_text(user_prompt).lower()
    if not normalized_prompt:
        return "intro"
    if _contains_story_agent_concierge_keyword(
        normalized_prompt, STORY_AGENT_CONCIERGE_TOPIC_SPOILER_KEYWORDS
    ):
        return "spoiler_guard"
    if _contains_story_agent_concierge_keyword(
        normalized_prompt, STORY_AGENT_CONCIERGE_TOPIC_MATCH_KEYWORDS
    ):
        return "match"
    if _contains_story_agent_concierge_keyword(
        normalized_prompt, STORY_AGENT_CONCIERGE_TOPIC_MOOD_KEYWORDS
    ):
        return "mood"
    if _contains_story_agent_concierge_keyword(
        normalized_prompt, STORY_AGENT_CONCIERGE_TOPIC_ENTRY_KEYWORDS
    ):
        return "entry"
    return "intro"


def _build_story_agent_concierge_signal_bundle(
    *,
    product_row: dict[str, Any],
    product_metadata: dict[str, Any] | None,
    taste_match: dict[str, Any] | None,
    recent_read_titles: list[str],
    reason_cards: list[StoryAgentReasonCard],
) -> dict[str, Any]:
    normalized_product_metadata = product_metadata or {}
    taste_tags = _normalize_story_agent_tag_list(
        normalized_product_metadata.get("taste_tags"), limit=4
    )
    worldview_tags = _normalize_story_agent_tag_list(
        normalized_product_metadata.get("worldview_tags"), limit=4
    )
    material_tags = _normalize_story_agent_tag_list(
        normalized_product_metadata.get("protagonist_material_tags"), limit=3
    )
    protagonist_type = _normalize_story_agent_concierge_text(
        normalized_product_metadata.get("protagonist_type")
    )
    protagonist_goal = _normalize_story_agent_concierge_text(
        normalized_product_metadata.get("protagonist_goal_primary")
    )
    mood = _normalize_story_agent_concierge_text(normalized_product_metadata.get("mood"))
    latest_episode_no = int(product_row.get("latestEpisodeNo") or 0)
    title = _normalize_story_agent_concierge_text(product_row.get("title")) or "이 작품"
...
def build_story_agent_concierge_actions() -> list[StoryAgentStarterAction]:
    return [
        {
            "label": "스포일러 없이 작품 소개",
...
def build_story_agent_concierge_cta_cards(*, product_id: int) -> list[StoryAgentCtaCard]:
    if product_id <= 0:
        return []
    return [
        {
...
def _build_story_agent_concierge_intro_reply(
    *,
...
def _build_story_agent_concierge_match_reply(
    *,
    signals: dict[str, Any],
) -> str:
...
def _build_story_agent_concierge_mood_reply(
    *,
    signals: dict[str, Any],
) -> str:
...
def _build_story_agent_concierge_entry_reply(
    *,
    signals: dict[str, Any],
) -> str:
...
def _build_story_agent_concierge_spoiler_guard_reply(
    *,
    signals: dict[str, Any],
) -> str:
...
def build_story_agent_concierge_reply(
    *,
    product_title: str,
    reason_cards: list[StoryAgentReasonCard],
    ...
    topic = _resolve_story_agent_concierge_topic(user_prompt)
    if topic == "match":
        return _build_story_agent_concierge_match_reply(signals=signals)
    if topic == "mood":
        return _build_story_agent_concierge_mood_reply(signals=signals)
    if topic == "entry":
        return _build_story_agent_concierge_entry_reply(signals=signals)
    if topic == "spoiler_guard":
        return _build_story_agent_concierge_spoiler_guard_reply(signals=signals)
    return _build_story_agent_concierge_intro_reply(signals=signals)


async def build_story_agent_concierge_payload(
    *,
    product_row: dict[str, Any],
    user_id: int | None,
    db: AsyncSession,
    ...
    recent_read_titles = await _get_story_agent_recent_read_titles(
        user_id=user_id,
        product_id=product_id,
        db=db,
    )
    reason_cards = _build_story_agent_concierge_reason_cards(
        product_row=product_row,
        product_metadata=product_metadata,
        taste_match=taste_match,
        recent_read_titles=recent_read_titles,
    )
    signal_bundle = _build_story_agent_concierge_signal_bundle(
        product_row=product_row,
        product_metadata=product_metadata,
        taste_match=taste_match,
        recent_read_titles=recent_read_titles,
    ...
    return {
        "scopeState": "none",
        "reasonCards": reason_cards,
        "actions": build_story_agent_concierge_actions(),
        "ctaCards": build_story_agent_concierge_cta_cards(product_id=product_id),
        "reply": build_story_agent_concierge_reply(
