...
STORY_AGENT_ALLOWED_GAME_MODES = {"ideal_worldcup", "vs_game"}
STORY_AGENT_ALLOWED_GAME_GENDER_SCOPES = {"male", "female", "mixed"}
...
STORY_AGENT_PENDING_GAME_CATEGORY = "__pending__"
STORY_AGENT_ALLOWED_VS_GAME_MATCH_MODES = {"direct_match", "criteria_match"}
STORY_AGENT_ALLOWED_READ_SCOPE_STATES = {"unknown", "none", "known"}
STORY_AGENT_RP_RECENT_FACT_LIMIT = 6


def _normalize_story_agent_string_list(raw_value: Any, *, limit: int = 20) -> list[str]:
    items: list[str] = []
...
def _normalize_story_agent_game_state(game_mode: str, raw_state: Any) -> dict[str, Any]:
    ...
    if game_mode == "ideal_worldcup":
        ...
        for pair in parsed.get("current_bracket") or []:
            if not isinstance(pair, (list, tuple)):
                continue
            normalized_pair = _normalize_story_agent_string_list(pair, limit=2)
        ...
        if requested_bracket_size not in {2, 4, 8}:
            requested_bracket_size = None
        return {
            "current_candidates": _normalize_story_agent_string_list(parsed.get("current_candidates"), limit=16),
            "current_bracket": current_bracket,
            "current_round": current_round,
            "current_match_index": current_match_index,
            "picks": _normalize_story_agent_string_list(parsed.get("picks"), limit=32),
            "used_pair_keys": _normalize_story_agent_string_list(parsed.get("used_pair_keys"), limit=128),
    ...
    if mode not in STORY_AGENT_ALLOWED_VS_GAME_MATCH_MODES:
        mode = None
    try:
        question_index = max(int(parsed.get("question_index") or 0), 0)
    except Exception:
        question_index = 0
    current_match = _normalize_story_agent_string_list(parsed.get("current_match"), limit=2)
    return {
        "mode": mode,
        "question_index": question_index,
        "answers": _normalize_story_agent_string_list(parsed.get("answers"), limit=64),
        "used_match_keys": _normalize_story_agent_string_list(parsed.get("used_match_keys"), limit=128),
        "used_question_keys": _normalize_story_agent_string_list(parsed.get("used_question_keys"), limit=128),
...
def _normalize_story_agent_games_memory(raw_value: Any) -> dict[str, Any]:
    parsed = raw_value if isinstance(raw_value, dict) else {}
    normalized_games: dict[str, Any] = {}
    for game_mode, raw_scopes in parsed.items():
        if game_mode not in STORY_AGENT_ALLOWED_GAME_MODES or not isinstance(raw_scopes, dict):
            continue
        scope_map: dict[str, Any] = {}
        for gender_scope, raw_categories in raw_scopes.items():
            if gender_scope not in STORY_AGENT_ALLOWED_GAME_GENDER_SCOPES or not isinstance(raw_categories, dict):
                continue
            category_map: dict[str, Any] = {}
            for category, raw_state in raw_categories.items():
                normalized_category = str(category or "").strip().lower()
                if normalized_category not in (STORY_AGENT_ALLOWED_GAME_CATEGORIES | {STORY_AGENT_PENDING_GAME_CATEGORY}):
                    continue
                category_map[normalized_category] = _normalize_story_agent_game_state(game_mode, raw_state)
            if category_map:
...
def _build_story_agent_game_context(
    *,
    game_mode: str | None,
    ...
    return {
        "mode": game_mode if game_mode in STORY_AGENT_ALLOWED_GAME_MODES else None,
        "gender_scope": game_gender_scope if game_gender_scope in STORY_AGENT_ALLOWED_GAME_GENDER_SCOPES else None,
        "category": game_category if game_category in STORY_AGENT_ALLOWED_GAME_CATEGORIES else None,
        "match_mode": game_match_mode if game_match_mode in STORY_AGENT_ALLOWED_VS_GAME_MATCH_MODES else None,
    }


def _clear_story_agent_game_context(session_memory: dict[str, Any]) -> dict[str, Any]:
...
def _normalize_story_agent_session_memory(raw_value: Any) -> dict[str, Any]:
    ...
    if read_scope_state not in STORY_AGENT_ALLOWED_READ_SCOPE_STATES:
    ...
    game_context = _build_story_agent_game_context(
    ...
    games = _normalize_story_agent_games_memory(parsed.get("games"))
    active_mode = str(parsed.get("active_mode") or "").strip().lower() or None
    if active_mode not in ({"rp"} | STORY_AGENT_ALLOWED_GAME_MODES):
        active_mode = None
    if active_mode in STORY_AGENT_ALLOWED_GAME_MODES and game_context.get("mode") != active_mode:
...
def _merge_story_agent_session_memory(
    *,
    ...
    if effective_game_mode not in STORY_AGENT_ALLOWED_GAME_MODES:
        effective_game_mode = None

    if effective_game_mode and has_game_inputs:
        next_memory["active_mode"] = effective_game_mode
        next_memory["game_context"] = _build_story_agent_game_context(
        ...
        if next_memory["game_context"].get("gender_scope"):
            ...
            if next_memory["game_context"].get("category"):
                ...
                next_state = _normalize_story_agent_game_state(effective_game_mode, existing_state)
        ...
        next_memory["games"] = _normalize_story_agent_games_memory(games)
...
def _serialize_story_agent_session_memory(session_memory: dict[str, Any]) -> str | None:
    ...
    if not has_rp_state and not has_game_state and not has_scope_state and not has_non_read_scope_state:
        return None
