...
async def get_websochat_game_candidate_profiles(
    *,
    product_id: int,
    db: AsyncSession,
) -> list[dict[str, Any]]:
    ...
    for row in result.mappings().all():
        ...
        if not scope_key:
            continue
        payload = _extract_websochat_json_object(str(row.get("summary_text") or "")) or {}
    ...
    for row in episode_summary_result.mappings().all():
        ...
        episode_no = _extract_websochat_episode_no_from_episode_summary_text(summary_text)
    ...
    filtered_candidates = sorted(
        [item for item in raw_candidates if _is_websochat_valid_game_candidate(item)],
        key=_sort_key,
    )
...
async def select_websochat_game_candidates(
    *,
