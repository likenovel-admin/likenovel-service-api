...
def _build_websochat_pair_key(name_a: str, name_b: str) -> str:
    return "::".join(sorted([str(name_a).strip(), str(name_b).strip()]))


def _is_websochat_valid_game_candidate(candidate: dict[str, Any]) -> bool:
...
def _resolve_websochat_pair_choice(user_prompt: str, pair: list[str]) -> str | None:
    if len(pair) != 2:
        return None
...
def _extract_websochat_episode_no_from_episode_summary_text(summary_text: str) -> int | None:
...
def _filter_websochat_worldcup_candidates_by_read_scope(
...
def _resolve_websochat_worldcup_bracket_size(
    *,
    read_episode_to: int,
...
def _infer_websochat_game_category_from_prompt(user_prompt: str) -> str | None:
...
def _extract_websochat_direct_match_scope_keys(
    *,
    user_prompt: str,
    candidates: list[dict[str, Any]],
) -> list[str]:
...
def _pick_websochat_unused_pair(
    candidates: list[dict[str, Any]],
    used_match_keys: list[str],
) -> list[dict[str, Any]]:
    used_set = set(_normalize_websochat_string_list(used_match_keys, limit=128))
    for left, right in combinations(candidates, 2):
        pair_key = _build_websochat_pair_key(left["display_name"], right["display_name"])
...
def _build_websochat_worldcup_round(
    candidate_names: list[str],
    used_pair_keys: list[str],
) -> tuple[list[list[str]], str] | tuple[None, None]:
    unique_candidates = _normalize_websochat_string_list(candidate_names, limit=8)
    if len(unique_candidates) < 2:
        return None, None
    used_set = set(_normalize_websochat_string_list(used_pair_keys, limit=128))
    if len(unique_candidates) == 2:
        pair_key = _build_websochat_pair_key(unique_candidates[0], unique_candidates[1])
        if pair_key in used_set:
            return None, None
    ...
    if len(unique_candidates) >= 8:
        ...
        while len(remaining) >= 2 and len(pairs) < 4:
            ...
            for idx, right in enumerate(remaining):
                pair_key = _build_websochat_pair_key(left, right)
    ...
    if len(target_candidates) == 3:
        ...
        for pair in possible_pairs:
            pair_key = _build_websochat_pair_key(pair[0], pair[1])
            if pair_key not in used_set:
    ...
    for pair in combinations(names, 2):
        ...
        remaining_key = _build_websochat_pair_key(remaining[0], remaining[1])
