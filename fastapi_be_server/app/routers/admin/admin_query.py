...
@router.get(
    ...
    tags=["CMS - 캐릭터구좌"],
    responses={200: {"description": "캐릭터구좌 카드 목록"}},
    dependencies=[Depends(analysis_logger)],
)
...
@router.get(
    ...
    tags=["CMS - 캐릭터구좌"],
    responses={200: {"description": "캐릭터구좌 등록 가능 작품 검색"}},
    dependencies=[Depends(analysis_logger)],
)
...
@router.get(
    "/main-character-slots/characters",
    tags=["CMS - 캐릭터구좌"],
    responses={200: {"description": "작품의 웹소챗 캐릭터 로스터(scope_key/display_name)"}},
    dependencies=[Depends(analysis_logger)],
)
async def main_character_slot_character_roster(
    product_id: int = Query(..., ge=1, description="작품 ID"),
    db: AsyncSession = Depends(get_likenovel_db),
    user: Dict[str, Any] = Depends(chk_cur_user),
):
    await check_user(kc_user_id=user.get("sub"), db=db, role="admin")
    return await main_character_slot_service.get_product_character_roster(
        product_id=product_id,
        db=db,
    )
